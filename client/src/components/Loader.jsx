export default function Loader() {
  return (
    <div className="flex-1 flex flex-col md:flex-row min-h-screen bg-black text-white overflow-hidden animate-pulse">
      {/* Sidebar Skeleton (hidden on mobile) */}
      <div className="hidden md:flex flex-col w-64 border-r border-white/10 p-6 space-y-6 h-screen">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div 
                key={i} 
                className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-4 flex flex-col"
                style={{ 
                  height: i % 3 === 0 ? '380px' : i % 2 === 0 ? '320px' : '350px' 
                }}
              >
                {/* Image Placeholder */}
                <div className="flex-1 bg-white/10 rounded-xl relative overflow-hidden">
